...
const Code: FC<CodeProps> = ({
	...
	const theme = applicationTheme === "light" ? themes.vsLight : darkTheme;

	return (
		<Highlight code={text} language={language} theme={theme}>
			{({ className, tokens, getLineProps, getTokenProps }) => (
				<pre
					className={`${className} transition-all w-fit bg-transparent duration-100 py-0 mb-3 no-scrollbar`}
					style={{
						maxHeight: show ? lines * 30 : 0,
						opacity: show ? 1 : 0,
					}}
				>
					{tokens.map((line, i) => {
